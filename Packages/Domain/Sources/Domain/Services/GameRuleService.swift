...
public struct GameRuleService {
    ...
    public func isValidGameSession(_ gameSession: GameSession) -> Bool {
        ...
        guard gameSession.duration >= GameSession.minDuration &&
              gameSession.duration <= GameSession.maxDuration else {
            return false
        }
        ...
        let playerNames = Set(gameSession.players.map { $0.name })
        let playerColors = Set(gameSession.players.map { $0.color })
        
        guard playerNames.count == gameSession.players.count &&
              playerColors.count == gameSession.players.count else {
    ...
    public func isValidInkSpotSize(_ size: Float) -> Bool {
        return size >= gameRules.inkSpotMinSize && size <= gameRules.inkSpotMaxSize
    }
...
extension GameRuleService: CustomStringConvertible {
    public var description: String {
        return "GameRuleService(rules: \(gameRules))"
    }
}